...
file_path = "D:/FCDados/[03] - Fatores, Estruturas de Controle e Funcoes/[01] - InputData/"

# Carregando os dados (Usando um timer para comparar o tempo de carregamento com diferentes funções)

# Usando read.csv2()
system.time(df_teste1 <- read.csv2(paste(file_path,"TemperaturasGlobais.csv", sep = "")))


# Usando read.table()
system.time(df_teste2 <- read.table(paste(file_path,"TemperaturasGlobais.csv", sep = "")))


# Usando fread()
?fread
system.time(df <- fread(paste(file_path,"/TemperaturasGlobais.csv", sep = "")))
